...
def verify_final():
    ...
    all_readiness = list(col.find().sort("sample_count", -1).limit(50))
    ...
    if valid_resilience:
        ...
        print(f"VERIFIED TOP ASN: AS{asn_id} ({org_name})")
        print(f"SAMPLE COUNT: {top_asn_doc.get('sample_count'):,}")
        print(f"IPV6 SCORE: {top_asn_doc.get('ipv6_capable')}%")
